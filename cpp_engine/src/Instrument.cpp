...
EuropeanOption::EuropeanOption(
    ...
    : option_type_(type),
      ...
      pricing_model_(PricingModel::BlackScholes),
      binomial_steps_(100),
      jump_intensity_(0.0),
      jump_mean_(0.0),
      jump_volatility_(0.0) {
    validateParameters();
}
...
PricingModel EuropeanOption::getPricingModel() const {
...
int EuropeanOption::getBinomialSteps() const {
    return binomial_steps_;
}

void EuropeanOption::setJumpParameters(double lambda, double jump_mean, double jump_vol) {
    if (lambda < 0.0) {
...
double EuropeanOption::getJumpIntensity() const {
    return jump_intensity_;
}
...
double EuropeanOption::getTimeToExpiry() const {
    return time_to_expiry_years_;
}
...
double EuropeanOption::priceBlackScholes(const MarketData& md) const {
    if (option_type_ == OptionType::Call) {
        return BlackScholes::callPrice(
            md.spot_price, strike_price_, md.risk_free_rate, 
            time_to_expiry_years_, md.volatility
        );
    } else {
        return BlackScholes::putPrice(
            md.spot_price, strike_price_, md.risk_free_rate, 
            time_to_expiry_years_, md.volatility
        );
    }
...
double EuropeanOption::priceJumpDiffusion(const MarketData& md) const {
    return JumpDiffusion::mertonOptionPrice(
        md.spot_price, strike_price_, md.risk_free_rate,
        time_to_expiry_years_, md.volatility,
        option_type_, jump_intensity_, jump_mean_, jump_volatility_
    );
}
...
double EuropeanOption::deltaBlackScholes(const MarketData& md) const {
    if (option_type_ == OptionType::Call) {
        return BlackScholes::callDelta(
            md.spot_price, strike_price_, md.risk_free_rate,
            time_to_expiry_years_, md.volatility
        );
    } else {
        return BlackScholes::putDelta(
            md.spot_price, strike_price_, md.risk_free_rate,
            time_to_expiry_years_, md.volatility
        );
    }
...
double EuropeanOption::theta(const MarketData& md) const {
    ...
    if (pricing_model_ == PricingModel::BlackScholes) {
        if (option_type_ == OptionType::Call) {
            result = BlackScholes::callTheta(
                md.spot_price, strike_price_, md.risk_free_rate,
                time_to_expiry_years_, md.volatility
            );
        } else {
            result = BlackScholes::putTheta(
                md.spot_price, strike_price_, md.risk_free_rate,
                time_to_expiry_years_, md.volatility
            );
        }
...
int AmericanOption::getBinomialSteps() const {
    return binomial_steps_;
}
