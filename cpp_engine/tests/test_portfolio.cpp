...
void test_add_single_instrument(TestSuite& suite) {
    suite.run_test("Add single call option", [&]() {
        ...
        suite.assert_equal(1, static_cast<double>(instruments.size()), 1e-10, "Portfolio size");
        suite.assert_equal(10, static_cast<double>(instruments[0].second), 1e-10, "Quantity");
    ...
    suite.run_test("Add single put option", [&]() {
        ...
        suite.assert_equal(1, static_cast<double>(instruments.size()), 1e-10, "Portfolio size");
        suite.assert_equal(5, static_cast<double>(instruments[0].second), 1e-10, "Quantity");
    });
}
...
void test_add_multiple_instruments(TestSuite& suite) {
    suite.run_test("Add multiple different instruments", [&]() {
        ...
        suite.assert_equal(3, static_cast<double>(instruments.size()), 1e-10, "Portfolio size");
        suite.assert_equal(10, static_cast<double>(instruments[0].second), 1e-10, "First quantity");
        suite.assert_equal(5, static_cast<double>(instruments[1].second), 1e-10, "Second quantity");
        suite.assert_equal(3, static_cast<double>(instruments[2].second), 1e-10, "Third quantity");
    });
}
...
void test_quantity_variations(TestSuite& suite) {
    ...
    suite.run_test("Zero quantity", [&]() {
        ...
        const auto& instruments = portfolio.getInstruments();
        suite.assert_equal(1, static_cast<double>(instruments.size()), 1e-10, "Should still add");
        suite.assert_equal(0, static_cast<double>(instruments[0].second), 1e-10, "Quantity is 0");
    });
    ...
    suite.run_test("Mixed long and short positions", [&]() {
        ...
        suite.assert_equal(10, static_cast<double>(instruments[0].second), 1e-10, "Long call");
        suite.assert_equal(-5, static_cast<double>(instruments[1].second), 1e-10, "Short put");
    });
}
...
void test_instrument_ownership(TestSuite& suite) {
    suite.run_test("Portfolio takes ownership of instruments", [&]() {
        ...
        auto option = std::make_unique<EuropeanOption>(OptionType::Call, 100.0, 1.0, "AAPL");
...
void test_large_portfolio(TestSuite& suite) {
    suite.run_test("Portfolio with many instruments", [&]() {
        ...
        for (int i = 0; i < 100; ++i) {
            portfolio.addInstrument(
                std::make_unique<EuropeanOption>(
                    OptionType::Call, 
                    100.0 + i, 
                    1.0, 
                    "AAPL"
        ...
        for (int i = 0; i < 100; ++i) {
            suite.assert_equal(i + 1, static_cast<double>(instruments[i].second), 1e-10);
        }
    });
}
